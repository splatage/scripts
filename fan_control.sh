...
BASELINE_C=${BASELINE_C:-50}    # target "quiet" temperature (°C)
UP_GAIN=${UP_GAIN:-2}           # % per +1°C above baseline
DOWN_GAIN=${DOWN_GAIN:-2}       # % per -1°C below baseline
...
# IBM: % → required hex step (safe table)
ibm_hex_for_pct(){
  local p=$1
...
echo "[$(date +'%F %T')] starting fan_control_simple (PID=$$) vendor=$VENDOR baseline=${BASELINE_C}C"
...
while true; do
  ...
  if [[ "$VENDOR" == "ibm" ]]; then
    ...
    echo "[$(date +'%F %T')] IBM: HOT=${HOT}°C  err=$((HOT-BASELINE_C))°C  -> ${CUR_PCT}%"
